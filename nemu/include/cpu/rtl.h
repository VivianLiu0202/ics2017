...
//pa2 level1 :finish update eflags function

static inline void rtl_eq0(rtlreg_t *dest, const rtlreg_t *src1)
{
  // dest <- (src1 == 0 ? 1 : 0)
  rtlreg_t temp = *src1 == 0 ? 1 : 0;
  rtl_lm(dest, &temp, 4);
}
...
{
  //获取指定宽度数值的最高位（Most Significant Bit，MSB）并将其存储到目标变量中
  // dest <- src1[width * 8 - 1]
