...
DecodeInfo decoding;      //存储当前正在解码的指令的相关信息。这包括操作码、操作数信息、指令指针等。
rtlreg_t t0, t1, t2, t3;  //这些是临时寄存器变量，用于在指令解码和执行过程中暂存数据。
const rtlreg_t tzero = 0; //一个常量寄存器，其值始终为0
 ...
 */
/* sign immediate */
//立即数的读取：用instr_fetch获取立即数，并把立即数转为带符号的整数
static inline make_DopHelper(SI)
{
  assert(op->width == 1 || op->width == 4);

  op->type = OP_TYPE_IMM; //将操作的类型设置为立即数类型，表示接下来的操作是针对立即数的。
